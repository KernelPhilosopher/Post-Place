document.addEventListener("DOMContentLoaded", () => {
  ...
  document.body.addEventListener("click", async (e) => {
    ...
    if (editBtn) {
      ...
      editModal.classList.add("visible"); // CORRECCIÓN: Usar clase
    }
  });
  ...
  saveEditBtn.addEventListener("click", async () => {
    ...
    try {
      ...
      editModal.classList.remove("visible"); // CORRECCIÓN: Usar clase
    } catch (error) {
  ...
  configBtn.addEventListener("click", () => {
    ...
    configModal.classList.add("visible"); // CORRECCIÓN: Usar clase
  });
  ...
  configForm.addEventListener("submit", async (e) => {
    ...
    try {
      ...
      userAvatarDisplay.textContent = LOGGED_IN_USER.nombre
        .substring(0, 1)
        .toUpperCase();
      configModal.classList.remove("visible"); // CORRECCIÓN: Usar clase
    } catch (error) {
  ...
  const updateOrAddPostInDOM = (postData) => {
    ...
    if (existingCard) {
      ...
      ["my-posts-list", "global-chat-list", "commented-posts-list"].forEach(
        (id) => {
          document
            .getElementById(id)
